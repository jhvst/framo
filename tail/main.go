...
func parseLog() {
	...
	result := fmt.Sprintf("Statement: With a confidence of 79 percent, human leg detected %.1f meters away, at an angle of %.1f degrees.", hypotenuse, angle)
	...
	// run through regression to find biggest chunks
	// as it is a map which has linestart and end, just find where
	// the difference is the biggest, and report that angle,
	// then calculate the distance to it
	// 	util.Regression([]byte(recentData))
}
